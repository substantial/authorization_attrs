...
describe AuthorizationAttrs do
  class FooAttrTestClass < ActiveRecord::Base
    def bar_id
      1
    end
  ...
  describe "acceptance specs", :db do
    ...
    it 'should return true if one of the attributes overlap' do
      ...
      authorized = AuthorizationAttrs.authorized?(:bazify, foo, user)
  ...
  describe ".authorized?" do
    ...
    it 'should return true if user attributes return :all' do
      allow_any_instance_of(Authorizations::FooAttrTestClassAuthorizations::UserAuthorizationAttrs)
    ...
    it 'should delegate the comparison of attributes to the storage strategy' do
      AuthorizationAttrs.authorized?(:bazify, foo, user)

      expect(AuthorizationAttrs::SqlDataStore).to have_received(:authorizations_match?)
        .with(record: foo, user_attrs: [{ bar_id: 1 }, { taco_id: 90 }])
    end
  end

  describe '.user_attrs' do
    it "should generate the authorization attributes data for that user and permission" do
      attrs = AuthorizationAttrs.user_attrs(:bazify, FooAttrTestClass, user)
